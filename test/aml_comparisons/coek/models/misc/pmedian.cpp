...
void pmedian_scalar(coek::Model& model, size_t N, size_t P)
{
    ...
    for (size_t n = 0; n < N; n++) {
        ...
        for (size_t m = 0; m < M; m++) model.add(x[n][m].bounds(0, 1).value(0));
    }

    std::vector<coek::Variable> y(N);
    for (size_t n = 0; n < N; n++) model.add(y[n].bounds(0, 1).value(0));
    ...
    // bound_y
    for (size_t n = 0; n < N; n++)
        for (size_t m = 0; m < M; m++) model.add(x[n][m] - y[n] <= 0);
