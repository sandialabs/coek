...
def fac_linear(G):
    ...
    model.Grid = pe.RangeSet(0, model.G)
    model.Facs = pe.RangeSet(1, model.F)
    model.Dims = pe.RangeSet(1, 2)

    model.d = pe.Var()
    model.y = pe.Var(model.Facs, model.Dims, bounds=(0.0, 1.0))
    model.z = pe.Var(model.Grid, model.Grid, model.Facs, within=Binary)
    ...
    def assmt_rule(mod, i, j):
        return LinearExpression(linear_coefs=ones, linear_vars=[mod.z[i,j,f] for f in mod.Facs]) == 1
