...
def fac_linear(G):
    ...
    def assmt_rule(mod, i, j):
        expr = LinearExpression(
            linear_coefs=[1 for f in mod.Facs],
            linear_vars=[mod.z[i,j,f] for f in mod.Facs],
            constant=-1
        )
        return (expr, 0)
    ...
    def quadrhs_rule(mod,i,j,f):
        expr = LinearExpression(
            linear_coefs=[1, -1, M],
            linear_vars=[mod.s[i,j,f], mod.d, mod.z[i,j,f]],
            constant=-M
        )
        return (expr, 0)
    ...
    def quaddistk1_rule(mod,i,j,f):
        expr = LinearExpression(
            linear_coefs=[1, 1],
            linear_vars=[mod.r[i,j,f,1], mod.y[f,1]],
            constant=-i/mod.G
        )
        return (expr, 0)
    ...
    def quaddistk2_rule(mod,i,j,f):
        expr = LinearExpression(
            linear_coefs=[1, 1],
            linear_vars=[mod.r[i,j,f,2], mod.y[f,2]],
            constant=-j/mod.G
        )
        return (expr, 0)
