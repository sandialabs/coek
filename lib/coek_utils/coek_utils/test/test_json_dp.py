...
def test_ip4():
    ...
    ip4(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"ip4": {"data": [[[1, "1"], [1, 1.1, "1"]]], "key_type": ["i", "s"], "param_type": ["i", "d", "s"]}}'
...
def test_ip6():
    ...
    ip6(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"ip6": {"data": [[1, [1, 1.1, "1"]]], "key_type": "i", "param_type": ["i", "d", "s"]}}'
...
def test_is3():
    ...
    is3(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"is3": {"data": [[[1, "1"], ["1", "2", "3"]]], "key_type": ["i", "s"], "set_type": "s"}}'

def test_is4():
    d = {}
    is4(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"is4": {"data": [[[1, "1"], ["1", "2", "3"]]], "key_type": ["i", "s"], "set_type": "s"}}'

def test_is5():
    d = {}
    is5(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"is5": {"data": [[1, ["1", "2", "3"]], [2, []]], "key_type": "i", "set_type": "s"}}'

def test_is6():
    d = {}
    is6(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"is6": {"data": [[1, []], [2, ["1", "2", "3"]]], "key_type": "i", "set_type": "s"}}'

def test_is7():
    d = {}
    is7(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"is7": {"data": [[1, [[1, 1], [2, 2]]]], "key_type": "i", "set_type": ["i", "i"]}}'

def test_is8():
    d = {}
    is8(d)
    ans = coek_utils.dump_to_json_string(d, sort_keys=True)
    assert ans == '{"is8": {"data": [[[1, "a"], [[1, "a", 1.1], [1, "a", 2.2]]], [[2, "b"], [[2, "b", 1.1], [2, "b", 2.2]]]], "key_type": ["i", "s"], "set_type": ["i", "s", "d"]}}'
