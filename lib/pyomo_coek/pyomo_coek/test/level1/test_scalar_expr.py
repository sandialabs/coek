...
def test_nestedSum(pyomo_module, var_a, var_b, var_c, var_d):
    ...
    e = e1 + 5
    #
    assert pe.to_list(e) == ["+", "a", "b", "5.000000"] or pe.to_list(e) == [
        "+",
        "5.000000",
        "a",
        "b",
    ...
    e = 5 + e1
    #
    assert pe.to_list(e) == ["+", "5.000000", ["+", "a", "b"]] or pe.to_list(e) == [
        "+",
        "a",
        "b",
        "5.000000",
    ...
    e = c + e1
    #
    assert pe.to_list(e) == ["+", "c", ["+", "a", "b"]] or pe.to_list(e) == ["+", "a", "b", "c"]
    ...
    e = e1 + e2
    #
    assert pe.to_list(e) == ["+", "a", "b", ["+", "c", "d"]] or pe.to_list(e) == [
        "+",
        "a",
        "b",
        "c",
...
def test_sumOf_nestedTrivialProduct(pyomo_module, var_a, var_b, var_c):
    ...
    e = e1 + e2
    #
    assert pe.to_list(e) == ["+", ["*", "5.000000", "a"], ["+", "b", "c"]] or pe.to_list(e) == [
        "+",
        "b",
        "c",
        ["*", "5.000000", "a"],
...
def test_negation_param(pyomo_module, param_p):
    ...
    #
    assert pe.to_list(e) == ["-", ["-", "p"]] or pe.to_list(e) == ["p"]
...
def test_negation_terms(pyomo_module, param_p, var_v):
    ...
    e = -e
    assert pe.to_list(e) == ["-", ["*", ["-", "p"], "v"]] or pe.to_list(e) == ["*", "p", "v"]
    #
    e = -5 * v
...
def test_trivialDiff(pyomo_module, var_a, param_p):
    ...
    # 0 - p*a
    e = 0 - p * a
    assert pe.to_list(e) == ["-", ["*", "p", "a"]] or pe.to_list(e) == ["*", ["-", "p"], "a"]
...
def test_nestedProduct2(pyomo_module, var_a, var_b, var_c, var_d):
    ...
    e3 = e1 + d
    e = e2 * e3
    assert pe.to_list(e) == ["*", ["+", "c", ["+", "a", "b"]], ["+", "a", "b", "d"]] or pe.to_list(
        e
    ) == ["*", ["+", "a", "b", "c"], ["+", "a", "b", "d"]]
...
def test_nestedProduct3(pyomo_module, var_a, var_b, var_c, var_d):
    ...
    e1 = 3 * b
    e = e1 * 5
    assert pe.to_list(e) == ["*", ["*", "3.000000", "b"], "5.000000"] or pe.to_list(e) == [
        "*",
        "15.000000",
        "b",
    ]
    ...
    e1 = 3 * b
    e = 5 * e1
    assert pe.to_list(e) == ["*", "5.000000", ["*", "3.000000", "b"]] or pe.to_list(e) == [
        "*",
        "15.000000",
        "b",
    ]
...
def test_trivialProduct(pyomo_module, var_a, param_p, param_q, param_r):
    ...
    e = a * p
    assert pe.to_list(e) == ["*", "a", "p"] or pe.to_list(e) == ["*", "p", "a"]
...
def test_trivialDivision(pyomo_module, var_a, param_p, param_q, param_r):
    ...
    #
    e = 0 / a
    assert pe.to_list(e) == ["0.000000"] or pe.to_list(e) == ["/", "0.000000", "a"]
    ...
    e = a / r
    assert pe.to_list(e) == ["/", "a", "r"] or pe.to_list(e) == ["*", ["/", "1.000000", "r"], "a"]
...
def test_trivialPow(pyomo_module, var_a, param_p, param_q, param_r):
    ...
    #
    e = a**0
    assert pe.to_list(e) == ["1.000000"] or pe.to_list(e) == ["pow", "a", "0.000000"]
    ...
    #
    e = 0**a
    assert pe.to_list(e) == ["0.000000"] or pe.to_list(e) == ["pow", "0.000000", "a"]

    e = 0.0**a
    assert pe.to_list(e) == ["0.000000"] or pe.to_list(e) == ["pow", "0.000000", "a"]

    e = 1**a
    assert pe.to_list(e) == ["1.000000"] or pe.to_list(e) == ["pow", "1.000000", "a"]
...
def test_entangled_test1(pyomo_module, var_a, var_b, var_c, var_d):
    ...
    assert pe.to_list(e2) == ["+", "c", ["+", "a", "b"]] or pe.to_list(e2) == ["+", "a", "b", "c"]
    assert pe.to_list(e3) == ["+", "d", ["+", "a", "b"]] or pe.to_list(e3) == ["+", "a", "b", "d"]
