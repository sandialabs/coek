...
class UnknownAD_Repn : public NLPModelRepn {
   ...
   public:
    ...
    void print_equations(std::ostream&)
    {
        throw std::runtime_error("Error accessing uninitialized NLPModel");
    }
   ...
   public:
    double compute_f(size_t) { throw std::runtime_error("Error accessing uninitialized NLPModel"); }
    ...
    CppAD::AD<double>& range, std::unordered_map<VariableTerm*,int>& _used_variables);
