   ...
   public:
    ...
    TYPE* cache(TYPE* ptr, int ctr = 0)
    {
        ...
        throw std::runtime_error(
            "ASTEnvironment::cache called with a pointer that is already cached.");
        return 0;
    }
    ...
    void uncache(TYPE* ptr)
    {
        ...
        if (curr == data.end())
            throw std::runtime_error(
                "ASTEnvironment::uncache called with a pointer that is not cached.");
        if (curr->second == 0) {
            data.erase(curr);
            delete tmp;
        }
    ...
    void own(TYPE* ptr)
    {
        ...
        cache_t::iterator curr = data.find(tmp);
        if (curr == data.end()) {
            // throw std::runtime_error("ASTEnvironment::own called with a pointer that is not
            // cached.");
    ...
    void disown(TYPE* ptr)
    {
        ...
        if (curr == data.end())
            throw std::runtime_error(
                "ASTEnvironment::disown called with a pointer that is not cached.");
    ...
    void free(TYPE* ptr)
    {
        ...
        if (curr == data.end())
            throw std::runtime_error(
                "ASTEnvironment::free called with a pointer that is not cached.");
        if (curr->second > 0) curr->second -= 1;
    }
