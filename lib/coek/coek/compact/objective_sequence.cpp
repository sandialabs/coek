...
class ObjectiveSeqIteratorRepn {
   ...
   public:
    ObjectiveSeqIteratorRepn(ObjectiveSequenceRepn* _seq, bool end) : seq(_seq)
    {
        done = end;
        if (!done) {
...
ObjectiveSeqIterator::reference ObjectiveSeqIterator::operator*() { return repn->operator*(); }
...
ObjectiveSeqIterator::pointer ObjectiveSeqIterator::operator->() { return repn->operator->(); }
...
ObjectiveSequence::ObjectiveSequence(const SequenceContext& context_, const Expression& expr,
                                     bool sense)
{
    repn = std::make_shared<ObjectiveSequenceRepn>(context_, expr, sense);
}

ObjectiveSeqIterator ObjectiveSequence::begin() { return ObjectiveSeqIterator(repn.get(), false); }

ObjectiveSeqIterator ObjectiveSequence::end() { return ObjectiveSeqIterator(repn.get(), true); }
