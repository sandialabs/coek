...
class ConstraintSeqIteratorRepn {
   ...
   public:
    ConstraintSeqIteratorRepn(ConstraintSequenceRepn* _seq, bool end) : seq(_seq)
    {
        done = end;
        if (!done) {
    ...
    reference operator*() { return converted_con; }

    const_reference operator*() const { return converted_con; }

    pointer operator->() { return &(converted_con); }

    const_pointer operator->() const { return &(converted_con); }
};
...
ConstraintSeqIterator::reference ConstraintSeqIterator::operator*() { return repn->operator*(); }
...
ConstraintSeqIterator::pointer ConstraintSeqIterator::operator->() { return repn->operator->(); }
...
ConstraintSeqIterator ConstraintSequence::end() { return ConstraintSeqIterator(repn.get(), true); }
