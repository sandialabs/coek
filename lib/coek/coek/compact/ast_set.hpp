...
class BaseSetExpression {
   ...
   public:
    BaseSetExpression() : _finite(false), _countable(false), initialized(false) {}

    virtual ~BaseSetExpression() {}

    virtual std::shared_ptr<SetIteratorRepnBase<std::vector<set_types>>> begin_NDiterator() = 0;
    virtual std::shared_ptr<SetIteratorRepnBase<std::vector<set_types>>> begin_NDiterator(
        const std::vector<IndexParameter>& index)
        = 0;
    ...
    bool countable() { return _countable; }
...
class SetRef : public BaseSetExpression {
   ...
   public:
    ...
    size_t dim() { return indexed_set->dim(); }

    void initialize() { throw std::runtime_error("Cannot initialize via a SetRef"); }

    size_t size() { throw std::runtime_error("Cannot get the size via a SetRef"); }

    std::shared_ptr<BaseSetExpression> set_union(
        const std::shared_ptr<BaseSetExpression>& lhs,
        const std::shared_ptr<BaseSetExpression>& rhs) const;

    std::shared_ptr<BaseSetExpression> set_intersection(
        const std::shared_ptr<BaseSetExpression>& lhs,
        const std::shared_ptr<BaseSetExpression>& rhs) const;

    std::shared_ptr<BaseSetExpression> set_difference(
        const std::shared_ptr<BaseSetExpression>& lhs,
        const std::shared_ptr<BaseSetExpression>& rhs) const;

    std::shared_ptr<BaseSetExpression> set_symmetric_difference(
        const std::shared_ptr<BaseSetExpression>& lhs,
        const std::shared_ptr<BaseSetExpression>& rhs) const;

    std::shared_ptr<BaseSetExpression> set_product(
        const std::shared_ptr<BaseSetExpression>& lhs,
        const std::shared_ptr<BaseSetExpression>& rhs) const;
    ...
    std::any value(size_t /*i*/) { throw std::runtime_error("Cannot call value() via a SetRef"); }
...
class SimpleSetIterator {
   ...
   public:
    ...
    SimpleSetIterator(const std::shared_ptr<SetIteratorRepnBase<TYPE>>& _repn) : repn(_repn) {}
    ...
    bool operator==(const SimpleSetIterator& other) const { return repn->equals(other.repn.get()); }
    ...
    reference operator*() { return repn->value(); }

    const_reference operator*() const { return repn->value(); }
};
...
class SimpleSet : public BaseSetExpression {
   public:
        ...
        const std::shared_ptr<BaseSetExpression>& rhs) const
    {
        if (rhs->dim() > 1)
            throw std::runtime_error(
                "Cannot take the intersection of sets with different dimensions.");
        ...
        const std::shared_ptr<BaseSetExpression>& rhs) const
    {
        if (rhs->dim() > 1)
            throw std::runtime_error(
                "Cannot take the difference of sets with different dimensions.");
        ...
        const std::shared_ptr<BaseSetExpression>& rhs) const
    {
        if (rhs->dim() > 1)
            throw std::runtime_error(
                "Cannot take the symmetric difference of sets with different dimensions.");
...
class FiniteSetIteratorRepn : public SetIteratorRepnBase<TYPE> {
   ...
   public:
    FiniteSetIteratorRepn(const typename std::vector<TYPE>::iterator& _iterator,
                          const typename std::vector<TYPE>::iterator& _end_iterator,
                          const IndexParameter& _index)
    ...
    TYPE& value() { return *iterator; }

    const TYPE& value() const { return *iterator; }
};
...
class FiniteNDSetIteratorRepn : public SetIteratorRepnBase<std::vector<set_types>> {
   ...
   public:
    FiniteNDSetIteratorRepn(const typename std::vector<TYPE>::iterator& _iterator,
                            const typename std::vector<TYPE>::iterator& _end,
                            const IndexParameter& _index)
...
class FiniteSimpleSet : public SimpleSet<TYPE> {
   ...
   public:
    ...
    {
        auto tmp
            = std::make_shared<FiniteNDSetIteratorRepn<TYPE>>(data.begin(), data.end(), indices[0]);
        return tmp;
    }
    std::shared_ptr<SetIteratorRepnBase<std::vector<set_types>>> end_NDiterator()
    {
    ...
    bool contains(const TYPE& arg) const { return !(set_data.find(arg) == set_data.end()); }
...
class SimpleSetUnion : public FiniteSimpleSet<TYPE> {
   ...
   public:
    SimpleSetUnion(const std::shared_ptr<SimpleSet<TYPE>>& _lhs,
                   const std::shared_ptr<SimpleSet<TYPE>>& _rhs)
        : lhs(_lhs), rhs(_rhs)
    {
...
class SimpleSetIntersection : public FiniteSimpleSet<TYPE> {
   ...
   public:
    SimpleSetIntersection(const std::shared_ptr<SimpleSet<TYPE>>& _lhs,
                          const std::shared_ptr<SimpleSet<TYPE>>& _rhs)
        : lhs(_lhs), rhs(_rhs)
    {
...
class SimpleSetDifference : public FiniteSimpleSet<TYPE> {
   ...
   public:
    SimpleSetDifference(const std::shared_ptr<SimpleSet<TYPE>>& _lhs,
                        const std::shared_ptr<SimpleSet<TYPE>>& _rhs)
        : lhs(_lhs), rhs(_rhs)
    {
...
class SimpleSetSymmetricDifference : public FiniteSimpleSet<TYPE> {
   ...
   public:
    SimpleSetSymmetricDifference(const std::shared_ptr<SimpleSet<TYPE>>& _lhs,
                                 const std::shared_ptr<SimpleSet<TYPE>>& _rhs)
        : lhs(_lhs), rhs(_rhs)
    {
        this->_finite = lhs->_finite and rhs->_finite;
...
class ListSet : public FiniteSimpleSet<TYPE> {
   ...
   public:
    ListSet(const std::initializer_list<TYPE>& _raw) : FiniteSimpleSet<TYPE>(), raw(_raw)
    {
        initialize();
    }
...
class SetSet : public FiniteSimpleSet<TYPE> {
   ...
   public:
    SetSet(const std::set<TYPE>& _raw) : FiniteSimpleSet<TYPE>(), raw(_raw) { initialize(); }
...
class VectorSet : public FiniteSimpleSet<TYPE> {
   ...
   public:
    VectorSet(const std::vector<TYPE>& _raw) : FiniteSimpleSet<TYPE>(), raw(_raw) { initialize(); }
...
class RangeSet : public SimpleSet<TYPE> {
   ...
   public:
    RangeSet(TYPE _start, TYPE _stop, TYPE _step = 1)
        : SimpleSet<TYPE>(), start(_start), stop(_stop), step(_step), tolerance(1e-7), nelements(0)
    {
        this->_finite = true;
        this->_countable = true;
        assert(stop > start);
    ...
    SimpleSetIterator<TYPE> end()
    {
        return SimpleSetIterator<TYPE>(
            std::make_shared<RangeSetIteratorRepn<TYPE>>(start, step, nelements));
    }
    ...
    bool contains(const TYPE& arg) const { return contains_value(arg, start, stop, step); }
...
class ProductSetIterator {
   ...
   public:
    ...
    ProductSetIterator(const std::shared_ptr<ProductSetIteratorRepn>& _repn) : repn(_repn) {}
...
class ProductSet : public BaseSetExpression {
   ...
   public:
    ProductSet() : BaseSetExpression() {}

    ProductSet(const std::shared_ptr<BaseSetExpression>& _lhs,
               const std::shared_ptr<BaseSetExpression>& _rhs);
    ProductSet(const std::shared_ptr<ProductSet>& _lhs,
               const std::shared_ptr<BaseSetExpression>& _rhs);
    ProductSet(const std::shared_ptr<BaseSetExpression>& _lhs,
...
class ProductSetUnion : public FiniteProductSet {
   ...
   public:
    ProductSetUnion(const std::shared_ptr<ProductSet>& _lhs,
                    const std::shared_ptr<ProductSet>& _rhs);
...
class ProductSetIntersection : public FiniteProductSet {
   ...
   public:
    ProductSetIntersection(const std::shared_ptr<ProductSet>& _lhs,
                           const std::shared_ptr<ProductSet>& _rhs);
...
class ProductSetDifference : public FiniteProductSet {
   ...
   public:
    ProductSetDifference(const std::shared_ptr<ProductSet>& _lhs,
                         const std::shared_ptr<ProductSet>& _rhs);
...
class ProductSetSymmetricDifference : public FiniteProductSet {
   ...
   public:
    ProductSetSymmetricDifference(const std::shared_ptr<ProductSet>& _lhs,
                                  const std::shared_ptr<ProductSet>& _rhs);
...
//
template <class TYPE>
std::shared_ptr<BaseSetExpression> SimpleSet<TYPE>::set_product(
    const std::shared_ptr<BaseSetExpression>& lhs,
    const std::shared_ptr<BaseSetExpression>& rhs) const
{
    if (rhs->dim() == 1) {
