...
#ifdef COEK_WITH_COMPACT_MODEL
void write_lp_problem(CompactModel& model, std::string& fname, std::map<int, int>& varmap,
                      std::map<int, int>& conmap);
void write_lp_problem_ostream(CompactModel& model, std::string& fname, std::map<int, int>& varmap,
                              std::map<int, int>& conmap);
#    ifdef WITH_FMTLIB
void write_lp_problem_fmtlib(CompactModel& model, std::string& fname, std::map<int, int>& varmap,
                             std::map<int, int>& conmap);
#    endif
#endif
...
Variable CompactModel::add_variable(const std::string& name, double lb, double ub, double value,
                                    bool binary, bool integer)
{
    Variable tmp(name, lb, ub, value, binary, integer);
    repn->variables.push_back(tmp);
    repn->variable_names.push_back("");
...
VariableMap CompactModel::add_variable(const Expression& lb, const Expression& ub,
                                       const Expression& value, bool binary, bool integer,
...
VariableMap CompactModel::add_variable(const std::string& name, const Expression& lb,
                                       const Expression& ub, const Expression& value, bool binary,
                                       bool integer, const SequenceContext& context)
{
    VariableSequence seq(context, lb, ub, value, binary, integer);
    repn->variables.push_back(seq);
...
ObjectiveMap CompactModel::add_objective(const Expression& expr, const SequenceContext& context,
                                         bool _sense)
{
...
ObjectiveMap CompactModel::add_objective(const std::string& /*name*/, const Expression& expr,
                                         const SequenceContext& context, bool _sense)
{
    ObjectiveSequence seq(context, expr, _sense);
    repn->objectives.push_back(seq);
...
ConstraintMap CompactModel::add_constraint(const std::string& /*name*/, const Constraint& expr,
                                           const SequenceContext& context)
{
    ConstraintSequence seq(context, expr);
    repn->constraints.push_back(seq);
...
{
    if (repn) return repn->num_variables();
    return 0;
}

size_t NLPModel::num_objectives() const
{
    if (repn) return repn->num_objectives();
    return 0;
}

size_t NLPModel::num_constraints() const
{
    if (repn) return repn->num_constraints();
    return 0;
}

size_t NLPModel::num_nonzeros_Jacobian() const
{
    if (repn) return repn->num_nonzeros_Jacobian();
    return 0;
}

size_t NLPModel::num_nonzeros_Hessian_Lagrangian() const
{
    if (repn) return repn->num_nonzeros_Hessian_Lagrangian();
    return 0;
}
...
{
    if (!repn) throw std::runtime_error("Calling write() for uninitialized NLPModel.");
...
void check_that_expression_variables_are_declared(Model& model,
                                                  const std::map<unsigned int, Variable>& varobj)
{
    ...
    */
    ...
    for (auto it = varobj.begin(); it != varobj.end(); it++) {
        ...
        if (tmp == model_ids.end()) {
            throw std::runtime_error("Model expressions contain variable '" + it->second.get_name()
