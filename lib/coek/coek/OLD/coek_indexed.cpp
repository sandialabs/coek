...
class VariableAssocArrayRepn {
   ...
   public:
    VariableAssocArrayRepn(const ConcreteSet& _arg) : concrete_set(_arg), call_setup(true)
    {
    ...
    std::string get_name(size_t index)
    {
        ...
        if (index >= names.size())
            throw std::runtime_error(
                "Asking for name with an index that is greater than the number of elements in the "
                "index set.");
        return names[index];
    }
    ...
    void setup()
    {
        ...
        for (size_t i = 0; i < concrete_set.size(); i++) {
            ...
            values.emplace_back(
                CREATE_POINTER(IndexedVariableTerm, CREATE_POINTER(ConstantTerm, lower),
                               CREATE_POINTER(ConstantTerm, upper),
                               CREATE_POINTER(ConstantTerm, value), binary, integer, i, this));
        }
        ...
        for (auto it = concrete_set.begin(); it != concrete_set.end(); ++it) {
            ...
            for (size_t j = 0; j < dim; j++) x[j] = vec[j];
            index[x] = i++;
        }
#else
        size_t i = 0;
        for (auto it = concrete_set.begin(); it != concrete_set.end(); ++it) index[*it] = i++;
#endif
    }
};
...
size_t VariableAssocArray::size() const { return repn->concrete_set.size(); }

std::vector<Variable>::iterator VariableAssocArray::begin() { return repn->values.begin(); }

std::vector<Variable>::iterator VariableAssocArray::end() { return repn->values.end(); }
...
VariableMap& VariableMap::value(double value)
{
    ...
    if (repn->values.size() > 0) {
        for (auto& var : repn->values) var.value(value);
    }
    return *this;
}
...
VariableMap& VariableMap::lower(double value)
{
    ...
    if (repn->values.size() > 0) {
        for (auto& var : repn->values) var.lower(value);
    }
    return *this;
}
...
VariableMap& VariableMap::upper(double value)
{
    ...
    if (repn->values.size() > 0) {
        for (auto& var : repn->values) var.upper(value);
    }
    return *this;
}
...
VariableMap& VariableMap::bounds(double lb, double ub)
{
    ...
    if (repn->values.size() > 0) {
        for (auto& var : repn->values) var.bounds(lb, ub);
    }
    return *this;
}
...
VariableMap& VariableMap::name(const std::string& name)
{
    repn->name = name;
    if (repn->values.size() > 0) {
        for (auto& var : repn->values) var.name(name);
    }
    return *this;
}
...
VariableMap& VariableMap::within(VariableTypes vtype)
{
    ...
    if (repn->values.size() > 0) {
        for (auto& var : repn->values) var.within(vtype);
    }
    return *this;
}
...
VariableArray variable(const std::vector<size_t>& shape)
{
    ...
    if (shape.size() > 1) {
        for (size_t i = 1; i < shape.size(); i++) arg *= coek::RangeSet(0, shape[i] - 1);
    }
    return VariableArray(arg);
}
...
VariableArray variable(const std::initializer_list<size_t>& shape)
{
    ...
    if (shape.size() > 1) {
        it++;
        for (; it != shape.end(); ++it) arg *= coek::RangeSet(0, *it - 1);
    }
    return VariableArray(arg);
}
