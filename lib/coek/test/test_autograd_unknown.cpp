...
TEST_CASE("unknown_ad", "[smoke]")
{
    SECTION("error1")
    {
        ...
        REQUIRE_THROWS_WITH(nlp.num_nonzeros_Jacobian(), "Error accessing uninitialized NLPModel");
        REQUIRE_THROWS_WITH(nlp.num_nonzeros_Hessian_Lagrangian(),
                            "Error accessing uninitialized NLPModel");

        std::vector<double> tmp1;
        REQUIRE_THROWS_WITH(nlp.set_variable_view(tmp1), "Error accessing uninitialized NLPModel");
        ...
        REQUIRE_THROWS_WITH(nlp.compute_H(tmp1, tmp1), "Error accessing uninitialized NLPModel");
