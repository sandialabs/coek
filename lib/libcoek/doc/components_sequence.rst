...
The key idea is to use quantifier expressions to express the elements
of an associative array.  This avoids the use of explicit looping,
and it allows for manipulation of the quantifier expression (e.g. for
...
variable binding is defered until the specification of the \textit{quantifier
expression}.  The ``set_element()`` function is used to specify a variable
that will be bound in a quantifier expression:
...
utility of explicit quantification in COEK is that the bound variables can be used in quantifier expressions
and to initialize component values.  For example, indexed sets can be used in quantifier 
...
quantifier expression is expanded.  Similarly, the ``ST()`` method can be
used to declare constraints on the bound variables that limit the range
of set elements generated by the quantifier expression.  For example:
...
Aside from component declarations, quantifier expressions can be used to provide a compact way to define complex expressions.
The ``Sum()`` function supports the declaration of a sum with a COEK expression that includes set elements, and a quantifier expression that binds those set elements.  For example:
...
The previous discussion of quantifier expressions illustrates the syntax needed to
declare variables and other components with quantified set notation.
...
Mutable parameters indexed with quantifier expressions can be declared in a similar manner:
...
Constraints indexed with quantifier expressions can be declared in a similar manner:
