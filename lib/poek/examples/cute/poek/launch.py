...
model.add_constraint( tt1 - 1.2*iw1 - 1.2*pw1 - 1.2*iw2 - 1.2*pw2 - 1.2*iw3 - 1.2*pw3 - 1.2*inw - 24.0 >= 0.0 )
model.add_constraint( tt1 - 1.4*iw1 - 1.4*pw1 - 1.4*iw2 - 1.4*pw2 - 1.4*iw3 - 1.4*pw3 - 1.4*inw - 28.0 <= 0.0 )
model.add_constraint( tt2 - 0.6*iw2 - 0.6*pw2 - 0.6*iw3 - 0.6*pw3 - 0.6*inw - 12.0 >= 0.0 )
model.add_constraint( tt2 - 0.75*iw2 - 0.75*pw2 - 0.75*iw3 - 0.75*pw3 - 0.75*inw - 15.0 <= 0.0 )
model.add_constraint( tt3 - 0.7*iw3 - 0.7*pw3 - 0.7*inw - 14.0>=0.0)
model.add_constraint( tt3 - 0.9*iw3 - 0.9*pw3 - 0.9*inw - 18.0<=0.0)
model.add_constraint( (mf1) * (inw) + 20.0*mf1 - iw1 - iw2 - pw2 - iw3 - pw3 - inw - 20.0 == 0)
...
model.add_constraint( pk.inequality(0.0, -32.0*(tt1 * bt1 * (log(mf1)) ) /pw1 - 32.0*(tt2 * bt2 * (log(mf2)) ) /pw2 - 32.0*(tt3 * bt3 * (log(mf3)) ) /pw3 - 35000.0, 15000.0) )
