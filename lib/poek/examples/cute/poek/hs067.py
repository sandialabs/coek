...
x1 = model.add_variable(lb=1.0e-5, ub=2.0e3)
x2 = model.add_variable(lb=1.0e-5, ub=1.6e4)
x3 = model.add_variable(lb=1.0e-5, ub=1.2e2)
...
while 1:
    ...
    y[2].value = (
        0.01 * x1.value * (112.0 + 13.167 * y[6].value - 0.6667 * y[6].value ** 2)
    )
...
while 1:
    y4old = y[4].value
    y[5].value = (
        86.35 + 1.098 * y[6].value - 0.038 * y[6].value ** 2 + 0.325 * (y[4].value - 89)
    )
    y[8].value = 3 * y[5].value - 133
    y[7].value = 35.82 - 0.222 * y[8].value
    y[4].value = 98000.0 * x3.value / (y[2].value * y[7].value + 1000 * x3.value)
    if abs(y4old - y[4].value) < 0.001:
        break

model.add_objective(
    -(0.063 * y[2] * y[5] - 5.04 * x1 - 3.36 * y[3] - 0.035 * x2 - 10 * x3)
)
