...
B = {}
for i in S:
    B[i] = 1.0 if i == 0 else B[i-1]*i

C = {i: 1.0 if i == 0 else (-1.0)**i*B[K]/(B[i]*B[K-i]) for i in S}

T = {i: (i-1.0)/(N+K-1.0) for i in R}

x = model.add_variable(index=R)

model.add_objective( sum(-(exp(T[i])+0.1*sin(i))*x[i] for i in R) +\
           sum(0.5*(exp(T[i])+0.1*sin(i))**2  for i in R) +\
