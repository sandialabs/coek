...
x = model.variable(index=R, value=0.0)

model.add( sum(-(cos(4*pi*T[i])+0.1*sin(i))*x[i] for i in R) - sum(0.5*(cos(4*pi*T[i])+0.1*sin(i))**2 for i in R) + sum(0.5*x[i]**2 for i in R) )
