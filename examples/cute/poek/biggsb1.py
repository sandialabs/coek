...
N = 1000
x = model.variable(list(range(1,N))

def f_rule(model):
model.add( (x[1]-1)**2 + sum((x[i+1]-x[i])**2 for i in range(1,N)) + (1-x[N])**2
f = Objective(rule=f_rule)

def cons1(model,i):
model.add( 0.0<=x[i]<=0.9
cons1 = Constraint(list(range(1,N-1),rule=cons1)
