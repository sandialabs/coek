...
model.add_objective( sum((-(x[1,TO[t]]+x[2,TO[t]])*k[1] + x[1,TO[t]] + x[2,TO[t]] + U[TO[t]]*k[1] - oc_init[t])**2 for t in range(1,NO+1)) )

for t in range(0, NT):
    model.add_constraint( H*(k[3]+k[4])*x[1,t] - H*k[5]*x[2,t] - H*U[t]*k[2] + x[1,t+1] - x[1,t] == 0 )
