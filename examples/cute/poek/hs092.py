...
def A_rule(model,j):
model.add( 2.0*sin(mu[j])/(mu[j] + sin(mu[j])*cos(mu[j]))
A = Param(N,value=A_rule)

def x_init_rule(model,i):
model.add( 0.5 * (-1)**(i+1)
x = model.variable(M,value=x_init_rule)

def rho(model,j):
model.add(  -(exp(-mu[j]**2 * sum(x[i]**2 for i in M))\
...
def obj_rule(model):
model.add( sum(x[i]**2 for i in M)
obj = Objective(rule=obj_rule,sense=minimize)

def cons1_rule(model):
model.add( sum(mu[i]**2 * mu[j]**2 * A[i] * A[j] * rho(model,i) * rho(model,j)\
